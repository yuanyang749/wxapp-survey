    ...
    try {
      ...
      const { data: survey, error: surveyError } = await supabase
        ...
        .select(`
          ...
          creator_id, status,
          profiles!creator_id (full_name, username, email, avatar_url)
        `)
        .eq('id', surveyId)
        .single()
      ...
      if (surveyError) {
        return {
          data: null,
          error: surveyError.code === 'PGRST116' 
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message?.includes('already participated')) {
          return { success: false, error: ERROR_MESSAGES.SURVEY.ALREADY_PARTICIPATED }
        }
